...
export const publicInformation: Record<PublicInformationKey, PublicInformationDocument> = {
  privacy: {
    ...
    sections: [
      ...
      { heading: 'Control and deletion', paragraphs: ['Download a progress backup from Progress → Your data before clearing browser storage. Use Reset data on that page to delete Pathfinder progress from the current browser. A saved feedback draft is removed after a successful copy action and can also be cleared with site data.'] },
  ...
  accessibility: {
    ...
    sections: [
      ...
      { heading: 'Motion and focus', paragraphs: ['Trace playback detects reduced-motion preferences and disables automatic animation. Dialogs use named close controls, and feedback status is announced through a polite live region.'] },
  ...
  data: {
    ...
    sections: [
      ...
      { heading: 'Delete local data', steps: ['Download a backup first if you may want the history later.', 'Open Progress and choose Reset data.', 'Confirm the reset. This clears Pathfinder progress and the active problem session from this browser. Clear site data in browser settings to remove any remaining feedback draft.'] },
